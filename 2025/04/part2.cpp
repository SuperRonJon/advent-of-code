...
public:
    ...
    const char& char_at(const std::size_t &col, const std::size_t &row) const {
        const std::size_t index = get_index(col, row);
    ...
    std::size_t remove_accessible_rolls() {
        std::size_t count{0};
        std::vector<std::size_t> indeces;

        for(std::size_t y = 0; y < height(); y++) {
            for(std::size_t x = 0; x < width(); x++) {
                if(char_at(x, y) == '@' && count_neighbors(x, y) < 4) {
                    indeces.push_back(get_index(x, y));
                    count++;
                }
            }
        }
        ...
        for(std::size_t i = 0; i < indeces.size(); i++) {
            m_values.at(indeces[i]) = '.';
        }
        return count;
    }
private:
    const std::size_t get_index(const std::size_t &col, const std::size_t &row) const {
...
int main(int argc, char **argv) {
    if(argc == 2) {
        ...
        std::cout << "Number of accessible rolls: " << total_count << "\n";
    }
    return 0;
}